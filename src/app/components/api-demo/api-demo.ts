...
type RequestAction =
  ...
  | { method: 'POST'; body: { title: string; body: string; userId: number } }
  | { method: 'PUT'; id: string; body: { id: number; title: string; body: string; userId: number } }
...
export class ApiDemo {
  ...
  public getOne(): void {
    this.action$.next({ method: 'GET_ONE', id: this.resourceId.trim() || '1' });
  }
  ...
  public delete(): void {
    this.action$.next({ method: 'DELETE', id: this.resourceId.trim() || '1' });
  }
