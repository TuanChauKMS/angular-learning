...
export class TaskService {
  ...
  public addTask(title: string, description: string, priority: TaskPriority, dueDate?: Date): void {
    const newTask: Task = {
      id: this.nextId++,
      title,
      description,
  ...
  public getTaskStats(): Observable<{ total: number; completed: number; active: number }> {
    return this.tasks$.pipe(
      map(tasks => ({
        total: tasks.length,
